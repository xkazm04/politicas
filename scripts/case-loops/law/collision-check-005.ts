...
const GROUPS_PATH = "docs/data-analysis/case-law/payloads/collision-groups-005.json";
const OUT_PATH = "docs/data-analysis/case-law/payloads/collision-report-v2.json"; // batch-004's v2 output — read-only reference, NOT overwritten (this script only writes OUT_PATH_V2)
const OUT_PATH_V2 = "docs/data-analysis/case-law/payloads/collision-report-v2-005.json";
...
// ---------- batch-005 P52: ranking-signal validation ----------
//
// batch-004's patterns.md finding: shared-§ COUNT does not discriminate confirmed collisions from
// incidental co-occurrence (confirmed pairs have a median of 4 shared §s, but three confirmed
// pairs had exactly 1, and the largest overlap in the whole 4-batch set was only a coordination-
// risk) — pre-partition it was an ANTI-predictor because omnibus contamination inflated overlap
// volume. The same reflection's patterns.md separately found that N-way clusters (4/18 clusters,
// 9/17 confirmed pairs) all fight over a § containing a bare monetary amount or allocation
// coefficient stated as a literal ("15 204 Kč", "350 000 Kč", a RUD revenue-formula coefficient).
// This is a cheap deterministic pre-rank candidate: does EITHER bill's shared-§ excerpt contain a
// Czech money/allocation literal? Flag it, then validate against the close-read classifications
// below (a candidate ranking must earn its keep the same way a triage signal does — P32).
// batch-005 audit fix: the original regex missed the bare-decimal-coefficient half of the P52
// hypothesis entirely (0 of 7 real "číslo „0,8“ nahrazuje číslem „0,4“"-shaped substitutions
// matched — [\d\s]* doesn't span a comma-decimal, and `\bkoeficient\b` misses every Czech
// inflection: koeficientu, koeficientem). Widened to also match a quoted decimal/percentage
// literal (the actual shape a Czech novelization uses for "replace this number with that one")
// and any "koeficient" stem regardless of case ending.
const MONEY_LITERAL_RE = /\d[\d\s]*\s?(Kč|%|promile)|koeficient\w*|„\s*\d+[,.]?\d*\s*(%|Kč)?\s*“/giu;
function hasMoneyLiteral(excerptA: string, excerptB: string): boolean {
  MONEY_LITERAL_RE.lastIndex = 0;
  const a = MONEY_LITERAL_RE.test(excerptA);
  MONEY_LITERAL_RE.lastIndex = 0;
  const b = MONEY_LITERAL_RE.test(excerptB);
  MONEY_LITERAL_RE.lastIndex = 0;
  return a || b;
}
...
async function runV2() {
  ...
  // ---------- P52 ranking: does the pair's shared-§ excerpt carry a money/allocation literal ----------
  const rankedPairs = allPairsV2.map((p) => {
    // find the excerpt text for this pair from groupReports (re-walk, cheap — small N)
    const g = groupReports.find((gr) => gr.lawRef === p.lawRef);
    const c = g?.collisions.find((cc) => cc.billA === p.billA && cc.billB === p.billB);
    const moneyHit = c ? c.sharedParagraphs.some((sp) => hasMoneyLiteral(sp.excerptA, sp.excerptB)) : false;
    return { ...p, sharedCount: p.paragraphs.length, moneyLiteral: moneyHit };
  });
  // sort: money-literal pairs first (P52 candidate signal), ties broken by MORE shared §s last
  // deliberately (batch-004 found raw count is not just non-predictive but slightly ANTI-
  // predictive pre-partition — after partitioning it is at best neutral, so it is the tiebreaker,
  // never the primary key).
  rankedPairs.sort((a, b) => (Number(b.moneyLiteral) - Number(a.moneyLiteral)) || (a.sharedCount - b.sharedCount));

  const report = {
    generatedAt: new Date().toISOString(),
    method:
      "batch-005: partitioned §-overlap (Q-law-10 method, batch-004) re-run on the batch-005 post-ingest/post-regen topology (collision-groups-005.json, 574 amends edges vs batch-004's 282-held/150-live). No 'diff against an unpartitioned original' this batch — the topology itself is new (145 multi-bill groups vs batch-004's 29), so there is no flat baseline to diff against; partitioning is applied directly as the only method (proven necessary by Q-law-10, not re-validated against a flat run here). Adds a P52 ranking signal: `moneyLiteral` — does the pair's shared §-excerpt (either bill) contain a Czech currency/percentage/coefficient literal — as the requested replacement for shared-§ COUNT (found NOT discriminative, in some regimes anti-predictive, by the batch-004 reflection). `rankedPairs` is `allPairs` sorted moneyLiteral-first; validate its discriminative power against the close-read classifications once done (see batch-005.md).",
    sourceGroups: GROUPS_PATH,
    priorBatchReference: OUT_PATH,
    stats: {
      groupsTotal: groups.length,
      billsInWorklist: worklist.length,
      billsFetched: extractions.size,
      billsSkipped: skips.length,
      totalCandidatePairs: allPairsV2.length,
      moneyLiteralFlagged: rankedPairs.filter((p) => p.moneyLiteral).length,
    },
    rankedPairs,
    skips,
    groups: groupReports,
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log(`candidate pairs (post-partition): ${allPairsV2.length}, money-literal-flagged: ${rankedPairs.filter((p) => p.moneyLiteral).length}`);
