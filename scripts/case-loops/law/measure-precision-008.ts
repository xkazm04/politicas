/* Case ③ Law loop — batch-008: full precision measurement on ALL regenerated `amends`
 * edges (docs/data-analysis/case-law/payloads/batch-008-amends-regen.json), reported as a metric
 ...
 * → docs/data-analysis/case-law/payloads/batch-008-precision-measurement.json
 */
import { existsSync, mkdirSync, readdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

const PAYLOAD = "docs/data-analysis/case-law/payloads/batch-008-amends-regen.json";
const OUT = "docs/data-analysis/case-law/payloads/batch-008-precision-measurement.json";
...
function main() {
  ...
  const out = {
    ...
    method:
      "Amending-context proxy (the SAME method batch-006's measure-precision-006.ts used, re-pointed at the batch-008 payload): for each regen edge (see summary.totalEdges below for the exact " +
        "population size measured THIS run — deliberately not hardcoded in this string, after a batch-008 self-review caught a stale count left over from an earlier pipeline iteration), search the bill's FULL cached text " +
    ...
    caveats: [
      ...
      "unresolvable_by_proxy CAN be large for title_fallback edges (citation source is the title prop, not necessarily reprinted in the body text) — in THIS run it happens to be 0 across all title_fallback edges (see bySource.title_fallback.unresolvable_by_proxy below for the exact count), meaning the batch-008 corpus's title-derived citations all also appear in the cached body text, but that is a property of this corpus, not a guarantee; treat a future run's 0 the same as a large figure — a coverage gap of the PROXY, not a precision defect " +
      ...
      "carried forward from batch-007's independent-audit finding N-D (this caveat predates batch-008's own audits, kept because it is still true of the same AMEND_VERB regex, unchanged this batch): AMEND_VERB includes 'se ruší' (repeal), so this proxy CANNOT distinguish a real amendment from a REPEAL clause — a repeal-target edge with its " +
