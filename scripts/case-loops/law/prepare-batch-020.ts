/* Case ③ Law loop — batch-018 verdict-target prep (derived from prepare-batch-020.ts).
 *
 * The 10 targets are the pending head after batch-019: tisky 35, 208, 48, 138, 130, 99, 199, 82, 42, 36 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-017 npx tsx scripts/case-loops/law/prepare-batch-020.ts
 */
...
const TARGET_CISLA = [35, 208, 48, 138, 130, 99, 199, 82, 42, 36];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-020-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 20, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-020 verdict targets → ${OUT_TARGETS}`);
