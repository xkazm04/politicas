/* Case ③ Law loop — batch-014 verdict-target prep (derived from prepare-batch-016.ts).
 *
 * The 10 targets are the pending head after batch-015: tisky 162, 228, 46, 186, 125, 106, 75, 88, 113, 214 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-016 npx tsx scripts/case-loops/law/prepare-batch-016.ts
 */
...
const TARGET_CISLA = [162, 228, 46, 186, 125, 106, 75, 88, 113, 214];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-016-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 16, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-016 verdict targets → ${OUT_TARGETS}`);
