/* Case ③ Law loop — batch-018 verdict-target prep (derived from prepare-batch-019.ts).
 *
 * The 10 targets are the pending head after batch-018: tisky 5, 235, 219, 163, 238, 57, 49, 15, 59, 243 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-017 npx tsx scripts/case-loops/law/prepare-batch-019.ts
 */
...
const TARGET_CISLA = [5, 235, 219, 163, 238, 57, 49, 15, 59, 243];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-019-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 19, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-019 verdict targets → ${OUT_TARGETS}`);
