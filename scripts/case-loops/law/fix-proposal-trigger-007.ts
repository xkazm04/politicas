/* Case ③ Law loop — batch-007: set-difference proposal trigger, re-pointed at the post-N1-fix
 ...
 * batch-007 census (amends-census.ts, after the ČÁST/bare-§ splitter + footnote-block + repeal-
 * exclusion fixes) instead of the pre-fix one.
 *
 *   npx tsx scripts/case-loops/law/fix-proposal-trigger-007.ts
 * → docs/data-analysis/case-law/payloads/batch-007-amended-laws-full-proposal-v2.json
 */
import { readFileSync, writeFileSync } from "node:fs";

const CENSUS_IN = "docs/data-analysis/case-law/payloads/batch-007-amends-census.json";
const OUT = "docs/data-analysis/case-law/payloads/batch-007-amended-laws-full-proposal-v2.json";
...
async function main() {
  ...
  const proposal = {
    ...
    method:
      "batch-007 re-run of batch-005 Q-law-11's set-difference trigger (recordedLaws set != realLaws set, either direction) against the batch-007 census (amends-census.ts post-N1/N2 fix: ČÁST/bare-§ splitter, footnote-block detection, repeal/non-amending-title exclusion). No new fetch.",
