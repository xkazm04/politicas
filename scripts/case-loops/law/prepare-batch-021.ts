/* Case ③ Law loop — batch-018 verdict-target prep (derived from prepare-batch-021.ts).
 *
 * The 10 targets are the pending head after batch-020: the final 12 — tisky 98, 129, 87, 116, 76, 52, 55, 6, 63, 101, 114, 144 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-017 npx tsx scripts/case-loops/law/prepare-batch-021.ts
 */
...
const TARGET_CISLA = [98, 129, 87, 116, 76, 52, 55, 6, 63, 101, 114, 144];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-021-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 21, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-021 verdict targets → ${OUT_TARGETS}`);
