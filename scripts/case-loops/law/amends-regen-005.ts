/* Case ③ Law loop — batch-005 Q-law-11: amends edge regeneration v2 (PREPARE only).
 *
 * Same method as amends-regen.ts (batch-004), re-run against the POST-INGEST universe:
 *   - PROPOSAL_IN now points at amended-laws-full-proposal-v2.json (Q-law-11's set-difference
 *     trigger, fix-proposal-trigger.ts — 61 bills vs the old 53, closing the tisk-219/222/243
 *     disjoint-statute blind spot the batch-004 reflection found).
 *   - run against .pglite-copy-law-005, which has the 188 missing-law-node ingest ALREADY applied
 *     (ingest-missing-laws.ts + _apply-missing-law-nodes-copy.ts) — so citations that were
 *     unresolved in batch-004 (no law node existed) now resolve, becoming real edges instead of
 *     missingLawNodeCensus entries.
 * Original file (batch-004, count-based trigger, pre-ingest universe) kept unchanged for history.
 *
...
const PROPOSAL_IN = "docs/data-analysis/case-law/payloads/amended-laws-full-proposal-v2.json";
const OUT = "docs/data-analysis/case-law/payloads/batch-005-amends-regen.json";
const IMPACT_OUT = "docs/data-analysis/case-law/payloads/batch-005-amends-regen-impact.md";
...
async function main() {
  ...
  interface EdgeProposal {
    ...
    to: string;
    ref: string;
    provenance: { track: "law"; pass: number; method: "deterministic"; ref: string };
      ...
      if (lawNodeId) {
        ...
        if (!edgeMap.has(key)) {
          edgeMap.set(key, {
            ...
            provenance: { track: "law", pass: 0, method: "deterministic", ref: "amends-regen-census" },
            source: refSource,
          });
        }
      } else {
  ...
  const out = {
    ...
    method:
      "batch-005 (Q-law-11 + Q-law-12 + audit remediation): for bills with a v2 (set-difference-triggered, amended-laws-full-proposal-v2.json) census_full proposal, use the UNION of the body-extracted citation list and the title-derived amended_laws prop (per-ref source tag preserved). For the other bills, fall back to title-derived only. Run against .pglite-copy-law-005, which already carries the 191 law nodes resolved by ingest-missing-laws.ts (188 round 1 + 3 round 2) — so citations unresolved in batch-004 now resolve. A citation becomes an edge ONLY IF a law node exists for it AND that node's e-Sbírka title marks it as an act of parliament (zákon/ústavní zákon), not a government regulation/decree/communiqué (D8 fix) — non-act targets are logged in excludedNonActRefs, never silently dropped or fabricated into an edge. Unresolved (no node at all) citations are counted in missingLawNodeCensus.",
    boundary: "PREPARE only — read-only against .pglite-copy-law-005, not applied to the live graph. Orchestrator executes the topology change; see diff-amends-regen-deletions.ts for the deletion-safety gate and batch-005.md/handoff.md for the full remediation list (Opus audit found this paired change-set NOT READY at the pre-remediation checkpoint — this run reflects the post-remediation state, still requires a fresh audit pass before apply).",
    stats: {
    ...
    caveats: {
      precisionNotFullyMeasured:
        "The Opus paired-landing audit (batch-005, pre-remediation checkpoint) found the census " +
        "body-extraction's single-subject-bill fallback (extractRealAmendedLaws, arts.length===0 " +
        "branch) was picking up FOOTNOTE citations instead of the real amending target on at " +
        "least 2 of 3 spot-checked bills (219, 243 — now fixed, see amends-census.ts's " +
        "isFootnoteLine) and flagged a ~6.3% proxy false-positive rate across all 574 edges that " +
        "was NOT re-measured after this remediation pass. Treat any single-citation edge to a " +
        "newly-ingested law node as lower-confidence than a multi-citation or pre-existing one " +
        "until a full precision audit re-runs (batch-006 candidate item).",
    },
    excludedNonActRefs,
