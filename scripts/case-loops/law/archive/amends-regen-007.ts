/* Case ③ Law loop — batch-007: amends edge regeneration v3 (PREPARE only).
 *
 * Same method as amends-regen-005.ts (batch-005), re-run against the POST-N1/N2-FIX census:
 *   - CENSUS_IN/PROPOSAL_IN now point at the batch-007 census (amends-census.ts, after the
 *     batch-006 independent audit's N1 fix — ČÁST/bare-§ splitter, alongside the original Čl.
 *     splitter — and N2/N4 fixes — footnote-BLOCK detection, a title-verb gate that excludes
 *     repeal/non-amending bills from the single-subject fallback).
 *   - run against .pglite-copy-law-007 (a fresh copy of .pglite-copy-law-005, which already
 *     carries the 187 missing-law-node ingest applied) — same resolution universe as batch-005/006.
 * batch-005/amends-regen-005.ts kept unchanged for history.
 *
 * Builds the full regenerated `amends` (bill → law) edge PROPOSAL set from the batch-007 census:
 *   - for bills with a batch-007 census_full proposal (set-difference trigger), use the UNION of
 *     the body-extracted citation list and the title-derived amended_laws prop;
 *   - for the other bills, fall back to title-derived only;
 ...
 * re-pointed at this payload).
 *
 *   PGLITE_PATH=./.pglite-copy-law-007 npx tsx scripts/case-loops/law/amends-regen-007.ts
 * → docs/data-analysis/case-law/payloads/batch-007-amends-regen.json
 * → docs/data-analysis/case-law/payloads/batch-007-amends-regen-impact.md
 */
...
const CENSUS_IN = "docs/data-analysis/case-law/payloads/batch-007-amends-census.json";
const PROPOSAL_IN = "docs/data-analysis/case-law/payloads/batch-007-amended-laws-full-proposal-v2.json";
const OUT = "docs/data-analysis/case-law/payloads/batch-007-amends-regen.json";
const IMPACT_OUT = "docs/data-analysis/case-law/payloads/batch-007-amends-regen-impact.md";
...
function buildImpactMd(out: {
  ...
  const lines: string[] = [];
  lines.push("# batch-007 — amends edge regeneration (post-N1/N2 census fix): impact analysis (prepare only)");
  lines.push("");
  lines.push(
