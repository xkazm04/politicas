/* Case ③ Law loop — batch-013 verdict-target prep (derived from prepare-batch-012.ts).
 *
 * The 10 targets are the pending head of the batch-013 re-triage (post census completion): tisky 205, 90, 68, 206, 73, 174, 53, 109, 62, 140 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-013 npx tsx scripts/case-loops/law/prepare-batch-013.ts
 */
...
const TARGET_CISLA = [205, 90, 68, 206, 73, 174, 53, 109, 62, 140];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-013-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 13, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-013 verdict targets → ${OUT_TARGETS}`);
