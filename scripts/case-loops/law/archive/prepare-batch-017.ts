/* Case ③ Law loop — batch-014 verdict-target prep (derived from prepare-batch-017.ts).
 *
 * The 10 targets are the pending head after batch-016: tisky 85, 12, 47, 131, 135, 193, 260, 50, 110, 72 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-017 npx tsx scripts/case-loops/law/prepare-batch-017.ts
 */
...
const TARGET_CISLA = [85, 12, 47, 131, 135, 193, 260, 50, 110, 72];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-017-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 17, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-017 verdict targets → ${OUT_TARGETS}`);
