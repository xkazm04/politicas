/* Case ③ Law loop — batch-014 verdict-target prep (derived from prepare-batch-014.ts).
 *
 * The 10 targets are the pending head after batch-013: tisky 141, 222, 257, 187, 83, 217, 89, 168, 25, 234 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-014 npx tsx scripts/case-loops/law/prepare-batch-014.ts
 */
...
const TARGET_CISLA = [141, 222, 257, 187, 83, 217, 89, 168, 25, 234];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-014-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 14, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-014 verdict targets → ${OUT_TARGETS}`);
