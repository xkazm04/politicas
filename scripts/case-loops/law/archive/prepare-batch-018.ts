/* Case ③ Law loop — batch-018 verdict-target prep (derived from prepare-batch-018.ts).
 *
 * The 10 targets are the pending head after batch-017: tisky 70, 261, 190, 78, 133, 107, 192, 61, 231, 153 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-017 npx tsx scripts/case-loops/law/prepare-batch-018.ts
 */
...
const TARGET_CISLA = [70, 261, 190, 78, 133, 107, 192, 61, 231, 153];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-018-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 18, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-018 verdict targets → ${OUT_TARGETS}`);
