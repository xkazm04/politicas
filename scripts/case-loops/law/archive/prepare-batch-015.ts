/* Case ③ Law loop — batch-014 verdict-target prep (derived from prepare-batch-015.ts).
 *
 * The 10 targets are the pending head after batch-014: tisky 167, 171, 218, 246, 74, 215, 145, 161, 232, 104 —
 ...
 *
 *   PGLITE_PATH=./.pglite-copy-law-015 npx tsx scripts/case-loops/law/prepare-batch-015.ts
 */
...
const TARGET_CISLA = [167, 171, 218, 246, 74, 215, 145, 161, 232, 104];
const OUT_TARGETS = "docs/data-analysis/case-law/payloads/batch-015-targets.json";
...
async function main() {
  ...
  writeFileSync(
    OUT_TARGETS,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), batch: 15, targets, gateScope: { knownLawRefsCount: knownLawRefs.size, knownIdsCount: knownIds.length }, knownLawRefs: [...knownLawRefs], knownIds },
      null,
      1,
    ),
  );
  console.log(`prepared ${targets.length} batch-015 verdict targets → ${OUT_TARGETS}`);
