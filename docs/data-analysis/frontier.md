...
| F5 | recompute-edge | refresh `co_votes_with` / `rebels_against` / cohesion on hlasovani re-ingest | 1 | deterministic edges go stale when `psp-hlasovani` re-ingests after the last `kg-compute` pass; re-run **`npm run da:kg-compute -- --commit`**. **NOT `--reset`** (corrected 2026-08-13 — this row prescribed it until then): `clearKg()` wipes EVERY node and edge, so on today's graph it deletes ~154 000 nodes / ~178 000 edges and writes ~1 000 back, taking /penize, /zakony, /denik and /graf with it. A recompute never needed it — the upsert replaces each claim in place and `mergeComputedNodeProps` keeps every prop kg-compute does not own. `kg-compute` now REFUSES a `--reset` that would drop kinds/rels it cannot rebuild, naming them (`guardKgReset`); `--supersede` is the deliberate override. | staleness-driven | seed §4.2 | open |
| F24 | recompute-edge | a conditionally-computed prop goes STALE instead of being dropped | 2 | the read-merge that fixed the erasure (D5, 2026-08-13) introduces its own smaller gap: if kg-compute stops emitting `rebellion_rate` for an MP who fell below `MIN_ELIGIBLE_VOTES`, the earlier pass's value survives under the earlier pass's provenance. Strictly better than the erasure it replaced, but a real stale-vintage risk. Fixing it means each writer DECLARING the prop keys it owns so an owned-but-absent key can be dropped — a per-writer ruling, not a merge-helper tweak. Disclosed in `mergeComputedNodeProps`'s doc comment and pinned by a test. | — | D5 | open |
