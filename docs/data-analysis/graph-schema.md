...
| `decides` | vote → bill | ⏳ **registered 2026-09-04, NOT YET WRITTEN** (writer dry-run only; 730 pairs / 473 roll calls / 77 prints would be emitted) | — | `kg-vote-bill-ingest.ts` — `vote_event(termPspId, sessionNo, agendaItem)` ⋈ `bod_schuze(bod, id_tisk)` through `schuze(id_org, schuze)`, i.e. the roll call and the print it decided. Props `votedOn`, `outcome`, `readingStage` (**null on 100 % today** — no dump column labels the reading; the prop exists so a later pass can fill it, and nothing renders a stage it does not have), `sessionNo`, `agendaItem`, `agendaLabel` (the item's own `zkratka`), `itemPrintCount`, `joinBasis: "schuze+bod"`. **The join basis is `pozvanka IS NULL` — the agenda AS TAKEN.** `bod_schuze` holds two numberings per sitting and the other one (`pozvanka = 1`, the pozvánka's per-block numbering) collides with nothing, links 40 % MORE roll calls, and is right 27,5 % of the time against the roll calls' own titles; the chosen pool is right 97,2 %. Many-to-many with the ambiguity counted, never resolved: 6 PSP10 agenda items name >1 print (2–18 each, all „písemné interpelace" blocks, none a law bill) and every candidate gets an edge carrying `itemPrintCount`. 1 602 of 2 075 valid roll calls link to no print (828 of them procedural, `bod = 0`) and are counted as `coverage.votesWithoutPrint`, never guessed from a title. See [[graph-log]] 2026-09-04 and [[frontier]] Q-law-2 |
