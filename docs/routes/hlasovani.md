...
**2026-09-07 — the published coverage counts are Czech-formatted (scan-sweep, copy-auditor).** The rules line passed `valid` / `tagged` / `candidates` to the catalog raw while the floors line beside it went through `f.int`; a chamber with 1 500 valid roll calls read „1500 platných hlasování". All three now format like every other figure on the page.

**2026-09-07 — one honest outcome renderer for the deník, the sál and the témata (scan-sweep, state-coverage).** `RealVoteLedger` and `RealChamberDetail` rendered every outcome that was not `accepted` through the „zamítnuto" branch — a default that named a value the catalog never checked; `VoteThemeFilter` had the honest known-set render with a local set of its own. `record/outcome.ts` now owns the vocabulary (`KNOWN_OUTCOMES`, `isKnownOutcome`, `outcomeTone`) and all three surfaces print an unknown stored token verbatim in a neutral tone. The kompas card fixed the same defect a round earlier with a local set (27a77dc); pointing it at this module is a follow-up in that context. `ledgerSource.test.ts` forbids the two-way ternary in all three files.
