// Text + vocabulary normalization for Czech civic data.
//
// WHY THIS EXISTS: Czech civic corpora carry diacritics in every human-facing
// key ("Nováková", "Poslanecký klub Starostové a nezávislí"), and search /
// entity-resolution has to match "Novakova" against "Nováková". Postgres
// solves this with the `unaccent` extension — which embedded engines (PGlite,
// SQLite) do not ship. So folding is done in application code at INGEST time
// and persisted into a `*_norm` column that carries a plain btree index.
// Never fold at query time: that defeats the index and re-introduces the
// dependency you are avoiding.
//
// The module also owns the documented psp.cz roll-call vocabularies
// (vote choice / outcome / kind) and the publisher's known data sentinels.
// Doctrine for all of them: missing beats wrong — an unknown code maps to an
